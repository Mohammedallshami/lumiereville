...
if(isset($_POST['order'])){
   ...
   if($check_cart->rowCount() > 0){
      ...
      $insert_order = $conn->prepare("INSERT INTO `orders`(userId, nom, numéroDeTéléphone, email, 	méthode, adresse, produitTotal, prixTotal) VALUES(?,?,?,?,?,?,?,?)");
...
<section class="checkout-orders">
   ...
   <h3>Vos commandes</h3>  <div class="display-orders">
  <?php
     $grand_total = 0;
     $cart_items[] = '';
  ...
  ?>
     ...
     <div class="grand-total">Total général : <span>$<?= $grand_total; ?>/-</span></div>
  </div>
  ...
  <div class="flex">
     ...
     <div class="inputBox">
        <span>Ville :</span>
        <input type="text" name="city" placeholder="Ex. Paris" class="box" maxlength="50" required>
     </div>
     <div class="inputBox">
        <span>État :</span>
     ...
     <div class="inputBox">
        ...
        <input type="number" min="0" name="pin_code" placeholder="Ex. 75001" min="0" max="999999" onkeypress="if(this.value.length == 6) return false;" class="box" required>
     </div>
  </div>
