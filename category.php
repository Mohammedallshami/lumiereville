...
<section class="products">
   ...
   <?php
     ...
     $select_products = $conn->prepare("SELECT p.proId, p.nom, p.prix, p.urlImage1, t.valeur, c.nom AS category  FROM catégories AS c, produits  AS p, tva AS t WHERE p.catId = c.catId AND p.tva = t.index AND c.nom LIKE '%{$category}%' ORDER BY p.proId ASC");
     $select_products->execute();
     if($select_products->rowCount() > 0){
      while($fetch_product = $select_products->fetch(PDO::FETCH_ASSOC)){
         $product_images = array($fetch_product['urlImage1']);
         $has_images = false;
